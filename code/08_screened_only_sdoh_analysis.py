...
def load_screened(path: Path) -> pd.DataFrame:
    """
    Load the journey table, inspect the SDOH screening column, and filter to
    journeys where the patient has at least one clean SDOH response.

    Adds:
      - core_need_count as clean integer
      - has_any_core_need as core_need_count >= 1
      - screened_core_need_group with screened-only labels
    """
    if not path.exists():
    ...
    if "core_need_count" not in jdf.columns:
        raise KeyError("core_need_count not found in parquet. Run script 07 first.")
    ...
    print(
        f"  Journeys with screening data : {n_screened:,}  "
        f"({round(n_screened / n_total * 100, 1)}%)"
    )
    print(
        f"  Journeys without (excluded)  : {n_dropped:,}  "
        f"({round(n_dropped / n_total * 100, 1)}%)"
    )

    # Clean core need fields. These are patient-level flags attached to journeys.
    screened["core_need_count"] = (
        pd.to_numeric(screened["core_need_count"], errors="coerce")
        .fillna(0)
        .astype(int)
    )

    for flag in CORE_NEED_FLAGS:
        if flag in screened.columns:
            screened[flag] = screened[flag].fillna(False).astype(bool)

    screened["has_any_core_need"] = screened["core_need_count"] >= 1
...
def save_group_counts(screened: pd.DataFrame) -> None:
    ...
    _sort_group(pd.DataFrame(rows)).to_csv(
        AUDIT_DIR / "screened_core_need_group_counts.csv",
        index=False,
    )
...
def save_type_by_need_group(screened: pd.DataFrame) -> None:
    """
    primary_journey_type × screened_core_need_group with:
    ...
    (
        ...
        .to_csv(
            AUDIT_DIR / "screened_journey_type_by_core_need_group.csv",
            index=False,
        )
    )
...
def save_type_by_any_need(screened: pd.DataFrame) -> None:
    ...
    _sort_type(ct).to_csv(
        AUDIT_DIR / "screened_journey_type_by_any_core_need.csv",
        index=False,
    )
...
def save_metric_summary(screened: pd.DataFrame) -> None:
    ...
    for group in SCREENED_GROUP_ORDER:
        ...
        rows.append({
            ...
            "median_first_to_second_gap_days": (
                round(float(gap.median()), 1) if len(gap) else np.nan
            ),
            "pct_hospital_involved": (
                _pct(int(grp["hospital_involved_journey"].sum()), n)
                if "hospital_involved_journey" in grp.columns else np.nan
            ),
            "pct_long_journey": (
                _pct(int(grp["long_journey"].sum()), n)
                if "long_journey" in grp.columns else np.nan
            ),
            "pct_one_visit": (
                _pct(int(grp["one_visit_journey"].sum()), n)
                if "one_visit_journey" in grp.columns else np.nan
            ),
        })

    _sort_group(pd.DataFrame(rows)).to_csv(
        AUDIT_DIR / "screened_core_need_metric_summary.csv",
        index=False,
    )
...
def save_specific_need_by_type(screened: pd.DataFrame) -> None:
    """
    For each of the six core need flags × each primary_journey_type:
      - n_journeys_with_need
      - n_type_screened_total
      - n_need_screened_total
      - pct_of_type_with_need
      - pct_of_need_in_this_type

    The earlier version failed because it used Series.where() with a scalar
    condition. This version uses an explicit scalar if/else for n_need_total.
    """
    type_totals = (
        screened
        .groupby("primary_journey_type")
        .size()
        .rename("n_type_screened_total")
    )
    ...
    for flag in CORE_NEED_FLAGS:
        ...
        flag_series = screened[flag].fillna(False).astype(bool)
        n_need_total = int(flag_series.sum())

        grp = (
            screened[flag_series]
            .groupby("primary_journey_type")
            .size()
        ...
        grp = grp.merge(
            type_totals.reset_index(),
            on="primary_journey_type",
            how="right",
        )

        grp["n_journeys_with_need"] = (
            grp["n_journeys_with_need"]
            .fillna(0)
            .astype(int)
        )
        ...
        if n_need_total > 0:
            ...
            grp["pct_of_need_in_this_type"] = 0.0
    ...
    if rows:
        ...
        (
            ...
            .to_csv(
                AUDIT_DIR / "screened_specific_need_by_journey_type.csv",
                index=False,
            )
        )
...
def save_notes(screened: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "    not captured by the six core flags, such as depression or alcohol use.",
        "",
        "  1 need   : core_need_count == 1",
        "  2-3 needs: core_need_count in [2, 3]",
        "  4+ needs : core_need_count >= 4, max possible is 6",
        "",
    ...
    for group in SCREENED_GROUP_ORDER:
        cnt = int(grp_vc.get(group, 0))
        lines.append(
            f"  {group:<35}: {cnt:>10,}  "
            f"({_pct(cnt, n, 1):>5.1f}% of screened journeys)"
        )
    ...
    lines += [
        ...
        dash,
        "  Alcohol Use, AUDIT-C composite",
        "  Depression, PHQ-2 / Edinburgh thresholds",
        "  Physical Activity, weekly volume",
        "  Social Connections, multi-item construct",
        "",
        "  INTERPRETATION REQUIREMENTS",
        dash,
        "",
        wrap(
            "1. All comparisons are within the screened subset only. Results "
            "cannot be generalised to the full journey cohort without "
            "acknowledging that screened patients may differ systematically "
            "from unscreened patients, for example by encounter type, time "
            "period, or care setting."
        ),
        "",
        wrap(
        ...
        wrap(
            ...
            "Frame this as 'patients with an ever-recorded positive screen,' "
            "not 'patients currently experiencing this need'."
        ),
        "",
        wrap(
            "4. Use careful language throughout: 'associated with', "
            "'observed among', 'linked to', or 'suggests'. Do not use "
...
def print_summary(screened: pd.DataFrame) -> None:
    ...
    print(
        f"  With any core need, positive screen   : {n_any:,}  "
        f"({_pct(n_any, n, 1)}%)"
    )
    print(
        f"  Screened, no positive core need       : {n_no:,}  "
        f"({_pct(n_no, n, 1)}%)"
    )
    ...
    for group in SCREENED_GROUP_ORDER:
        cnt = int(grp_vc.get(group, 0))
        print(f"  {group:<35}: {cnt:>10,}  ({_pct(cnt, n, 1):>5.1f}%)")
    ...
    if "hospital_involved_journey" in screened.columns:
        for group in SCREENED_GROUP_ORDER:
            grp = screened[screened["screened_core_need_group"] == group]
            n_grp = len(grp)
            n_hospital = int(grp["hospital_involved_journey"].sum())
            print(
                f"  {group:<35}: {_pct(n_hospital, n_grp, 1):>5.1f}%  "
                f"({n_hospital:,} / {n_grp:,})"
            )

    print("\n  primary_journey_type distribution by screened_core_need_group")
    print(
        f"  {'Type':<28}  {'No pos need':>11}  {'1 need':>8}  "
        f"{'2-3':>7}  {'4+':>6}  {'Total':>8}"
    )
    print(f"  {'-' * 72}")
    ...
    for ptype in PRIMARY_TYPE_ORDER:
        ...
        vals = {
            group: int((sub["screened_core_need_group"] == group).sum())
            for group in SCREENED_GROUP_ORDER
        }
